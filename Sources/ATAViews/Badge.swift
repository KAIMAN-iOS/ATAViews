...
public class Badge: UILabel {
    ...
    public static var size: CGFloat = 25
    ...
    public override func layoutSubviews() {
        ...
        snp.makeConstraints { [weak self] in
            ...
            $0.width.equalTo(Badge.size)
        }
        font = .applicationFont(ofSize: 12)
    ...
    public func update(_ count: Int) {
        text = "\(count)"
        print("📚 update for \(count) - isHidden \(isHidden) on \(Thread.current)")
        guard (count == 0 && isHidden == false) || (count > 0 && isHidden == true) else { return }
        print("📚 update hidden state to \(count == 0)")
//        isHidden = false
        isHidden = count == 0
    }
}
public class ATABadgeView: BadgeHub {
    public static var circleBackgroundColor: UIColor = .red
    public static var textColor: UIColor = .white
    public var circleBackgroundColor: UIColor?  {
        didSet {
            setCircleColor(circleBackgroundColor ?? ATABadgeView.circleBackgroundColor,
                           label: textColor ?? ATABadgeView.textColor)
        }
    }
    ...
    public override init(view: UIView) {
        ...
        setCircleColor(ATABadgeView.circleBackgroundColor, label: ATABadgeView.textColor)
        setCountLabelFont(.applicationFont(ofSize: 10))
        scaleCircleSize(by: 0.7)
        moveCircleBy(x: 0, y: 5)
    }
}
